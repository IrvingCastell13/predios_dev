...
const fetchChartData = async () => {
  ...
  try {
    ...
    const seriesData = data.map((d) => [
      allX.indexOf(d.x),
      allY.indexOf(d.y),
      d.v, // El valor (100 para 'con archivo', otro para 'sin archivo')
    ]);
    ...
    option.value = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          const status = value === 100 ? "Con archivo adjunto" : "Sin archivo";
          return `<b>${yLabel}</b><br/>${xLabel}<br/>${status}`;
        },
      },
      grid: {
        left: "2%",
      ...
      visualMap: {
        ...
        bottom: "10%",
        pieces: [
          { value: 100, label: "Con archivo", color: "#4ade80" },
          { lt: 100, label: "Sin archivo", color: "#f87171" },
        ],
        calculable: false,
      },
      ...
      series: [
        {
          name: "Estado de Archivo",
          type: "heatmap",
          data: seriesData,
          // --- CAMBIO CLAVE: Añadir borde a cada celda ---
          itemStyle: {
            borderColor: "#fff",
            borderWidth: 2,
          },
