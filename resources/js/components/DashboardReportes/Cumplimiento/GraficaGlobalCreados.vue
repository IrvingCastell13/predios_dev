...
// --- PROPS ---
// Mismos props que la gráfica original para mantener la reactividad a los filtros
const props = defineProps({
  apiUrl: { type: String, required: true },
  chartTitle: {
    type: String,
    default: "Cumplimiento Global (Documentos Creados)",
  },
...
const processChartData = (data) => {
  ...
  // ✨ ¡CAMBIO AQUÍ! Sumamos los valores de todos los predios.
  const totalPredios = creadosDataset.data.length;
  const sumaCreados = creadosDataset.data.reduce(
    (acc, value) => acc + (parseFloat(value) || 0),
    0
  );

  // Calculamos el promedio global de cumplimiento y el faltante.
  const promedioGlobalCreados =
    totalPredios > 0 ? sumaCreados / totalPredios : 0;
  const promedioGlobalFaltante = 100 - promedioGlobalCreados;

  const chartData = [
    { value: promedioGlobalCreados.toFixed(2), name: "% Creados" },
  ...
  chartOptions.value = {
    ...
    legend: {
      ...
      left: "left",
      top: "middle",
      data: ["% Creados", "% Faltante"],
    },
    series: [
      {
        name: "Cumplimiento Creados",
        type: "pie",
        ...
        itemStyle: {
          color: (params) =>
            params.name === "% Creados" ? "#4ade80" : "#f87171",
        },
        label: {
          show: true,
          position: "inside",
...
// --- WATCHER ---
// Observa los cambios en los filtros para recargar los datos
watch(
  () => [
    props.idPredios,
    props.idGrupos,
