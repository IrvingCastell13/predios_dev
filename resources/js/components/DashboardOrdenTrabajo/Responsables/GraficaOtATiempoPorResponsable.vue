...
const fetchData = async () => {
  ...
  try {
    const response = await axios.get(
      "/api/bi/ordenes-trabajo/a-tiempo-por-responsable",
      {
        params: {
...
const processChartData = (data) => {
  const responsables = data.map((item) => item.responsable);

  // --- LÓGICA DE SCROLL VERTICAL ---
  const dataZoomConfig = [];
  const yAxisThreshold = 6; // Mostrar 6 responsables antes de activar el scroll
  ...
  if (responsables.length > yAxisThreshold) {
    dataZoomConfig.push({
      ...
      yAxisIndex: 0,
      start: 0,
      end: (yAxisThreshold / responsables.length) * 100,
      right: "15px",
      top: 70,
      bottom: 40,
      width: 8,
  ...
  // --- FIN DE LÓGICA DE SCROLL ---

  chartOptions.value = {
    title: { text: "Cumplimiento de Inicio por Responsable", left: "center" },
    ...
    series: [
      {
        ...
        data: data.map((item) => item.a_tiempo),
      },
      {
        ...
        data: data.map((item) => item.fuera_tiempo),
      },
    ],
  };
};
