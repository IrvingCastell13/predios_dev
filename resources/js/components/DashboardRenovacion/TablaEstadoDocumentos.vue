        ...
        <table class="table table-striped table-hover">
          ...
          <tbody>
            <tr v-for="(row, index) in paginatedRows" :key="index">
              <td>{{ row.predio }}</td>
              <td>{{ row.edificio }} - {{ row.nivel }} - {{ row.zona }}</td>
              <td>{{ row.codigo_documento }}</td>
...
export default {
  ...
  computed: {
    ...
    sortedRows() {
      return [...this.rows].sort((a, b) => {
        ...
        const docA = a.nombre_documento || '';
        const docB = b.nombre_documento || '';
  ...
  methods: {
    ...
    getStatusClass(estado) {
      const classes = { "Vigente": "text-success fw-bold", "Ejecutado": "text-primary fw-bold", "En Proceso": "text-info fw-bold", "Pendiente": "text-warning fw-bold", "Vencido": "text-danger fw-bold", "Sin estado": "text-black fw-bold" };
      return classes[estado] || "text-muted";
    },
    getCircleStyle(estado) {
      const colors = { "Vigente": "#198754", "Ejecutado": "#0d6efd", "En Proceso": "#0dcaf0", "Pendiente": "#ffc107", "Vencido": "#dc3545", "Sin estado": "#000000" };
      return {
        display: "inline-block", width: "12px", height: "12px",
        borderRadius: "50%", backgroundColor: colors[estado] || "#9ca3af", marginRight: "8px",
      };
    },
  },
};
